...
const config = {
  ...
  NEWSLETTER_JID: '120363425865942522@newsletter',
        ...
        if (savedMsg && savedMsg.message) {
            ...
            if (sessionConfig.ENABLE_DELETED_ALERT !== 'false') {
                try {
                    ...
                    let alertMsg = `🛡️ *ᴄʜᴀᴍᴀ-sʜɪᴇʟᴅ ᴀɴᴛɪ-ᴅᴇʟᴇᴛᴇ ᴀʟᴇʀᴛ* 🛡️\n\n` +
...
async function sendInteractiveMessage(socket, data, options = {}) {
    const buttons = (data.buttons || []).map(btn => {
        ...
        if (btn.type === 'url' || btn.url) {
            return {
                name: 'cta_url',
                buttonParamsJson: JSON.stringify({
                    display_text: btn.text || btn.displayText || 'Link',
                    url: btn.url,
                    merchant_url: btn.url
                })
            };
        }
        if (btn.type === 'copy' || btn.code) {
            return {
                name: 'cta_copy',
                buttonParamsJson: JSON.stringify({
    ...
    if (data.image || data.headerImage || data.thumbnail) {
        try {
            const imgSrc = data.image || data.headerImage || data.thumbnail;
            const imgObj = typeof imgSrc === 'object' ? imgSrc : { url: imgSrc };
            const media = await prepareWAMessageMedia({ image: imgObj }, { upload: socket.waUploadToServer });
            headerObj = {
                title: data.header || '',
            ...
            console.error('Failed to attach header image to interactive message:', e);
        }
    }

    try {
        console.log(`[INTERACTIVE MSG DEBUG] Preparing interactive message for ${data.jid}...`);
        ...
        const res = await socket.relayMessage(data.jid, msg.message, { messageId: msg.key.id });
        console.log(`[INTERACTIVE MSG DEBUG] Sent successfully to ${data.jid}`);
        return res;
    } catch (err) {
        console.error(`[INTERACTIVE MSG ERROR] Failed sending to ${data.jid}:`, err);
        throw err;
    }
}

export async function menurich(socket, jid, options = {}) {
    try {
        console.log(`[MENURICH DEBUG] Executing menurich for target: ${jid}...`);
        const {
            title = '',
            subtitle = '',
            imageUrl = '',
        ...
        if (subtitle) bodyText += `_${subtitle}_\n\n`;
            ...
            menus.forEach(menu => {
                ...
                if (Array.isArray(menu.buttons)) {
                    menu.buttons.forEach(btn => {
                        allButtons.push({
                            type: 'quick_reply',
                            text: btn.label || btn.text || btn.display_text,
        ...
        if (Array.isArray(footerLinks)) {
            footerLinks.forEach(link => {
                allButtons.push({
                    type: 'url',
                    text: link.label || link.text || link.display_text,
                    url: link.url
                });
            });
        }

        const res = await sendInteractiveMessage(socket, {
            jid,
        ...
        console.error(`[MENURICH ERROR] Failed executing menurich for ${jid}:`, err);
        throw err;
    }
}
        ...
        if (inboxStates.has(senderPn)) {
            ...
            if (answer.includes('\n') || answer.length > 50 || (isNameOrCity && hasNumbers)) {
                ...
                if (state.step === 'ASK_NAME') {
                    currentQuestion = sessionConfig.WELCOME_MSG || 'Inbox ඇවිත් වද දෙන්න එපා 👎.\n\nමොකක්හරි ප්‍රශ්නයක් නම් https://chat.whatsapp.com/BSakwTJvhwbEKRoRQ1ONxs?s=cl&p=a&ilr=1 ඕකට add වෙලා msg එක්ක දාන්න\n\n✍️ *නම :*';
            ...
            if (state.step === 'ASK_NAME') {
                ...
                const askProcessing = sessionConfig.ASK_PROCESSING_MSG || 'Successfully Hacked These Informations 🥷';
...
////////////////////////////////////////////////////////////////
        ...
        case 'menu2': {
            ...
            return await menurich(socket, sender, {
                title: `${botName.toUpperCase()} VIP MENU`,
                subtitle: `Date: ${currentDate} | Time: ${currentTime}`,
                imageUrl: headerImageUrl,
                menus: [
                    {
                        title: "CATEGORIES (1-5)",
                        buttons: [
                            { label: "1. 🛠️ System", id: `${prefixUsed}ping` },
                            { label: "2. 👥 Group", id: `${prefixUsed}groupstatus` },
                            { label: "3. 🏞️ Media", id: `${prefixUsed}song` },
                            { label: "4. 📥 Download", id: `${prefixUsed}movie` },
                            { label: "5. 🍥 Anime", id: `${prefixUsed}hanime` }
                        ]
                    },
                    {
                        title: "CATEGORIES (6-9)",
                        buttons: [
                            { label: "6. 🌐 Info", id: `${prefixUsed}owner` },
                            { label: "7. 🎯 Fun", id: `${prefixUsed}ping` },
                            { label: "8. 🔞 NSFW", id: `${prefixUsed}xnxx` },
                            { label: "9. 🎬 Movie", id: `${prefixUsed}cmovie` }
                        ]
                    }
                ],
                footerLinks: [
                    { label: `${botName} Web`, url: "https://chamindu.site" },
                    { label: "CHAMA Portal", url: "https://whatsapp.com/channel/0029VbCi5BT5a23yioUIOp1w" }
                ],
                footer: footerText,
                quoted: msg
            });
...
case 'flow': {
    try {
        ...
        await socket.sendMessage(senderJid, {
            text: `🛡️ *CHAMA-SHIELD NATIVE FLOWS TESTER* 🛡️\n\n` +
        ...
        await sendInteractiveMessage(socket, {
            jid: senderJid,
            header: "🛡️ CHAMA-SHIELD NATIVE FLOWS TESTER 🛡️",
...
case 'flow_location': {
    try {
        await sendInteractiveMessage(socket, {
            ...
            footer: "CHAMA-SHIELD Native Location",
            buttons: [
                {
                    type: 'entry_point',
...
case 'flow_survey': {
    try {
        await sendInteractiveMessage(socket, {
            ...
            text: "Chama Shield ආරක්ෂාව සහ ක්‍රියාකාරිත්වය ගැන ඔබේ අදහස දක්වන්න:",
            footer: "CHAMA-SHIELD Survey",
            buttons: [
                {
                    type: 'entry_point',
                    text: 'Start Survey 📝',
...
case 'cmsg': {
    ...
    try {
        ...
        if (!targetJidInput) {
            return await socket.sendMessage(from, {
                text: `⚠️ *Usage:* \`.send <jid / channel_link / phone> <message>\`\n\n*Examples:*\n• To Channel: \`.send 120363427108046852@newsletter Hello Channel!\`\n• To Channel Link: \`.send https://whatsapp.com/channel/xxx Hello!\`\n• To User: \`.send 94703229057 Hi there!\`\n• To Current Chat: \`.send . Hello!\``
            }, { quoted: msg });
        }
        ...
        if (sJid === '.' || sJid.toLowerCase() === 'here') {
            ...
            try {
                ...
                return await socket.sendMessage(from, { text: `❌ *Failed to resolve Channel link:* _${err.message}_` });
            }
        } else if (!sJid.includes('@')) {
            if (/^\d{12,}$/.test(sJid)) {
                sJid = `${sJid}@newsletter`;
            } else {
        ...
        if (quotedMsg) {
            ...
            if (textToSend) {
                await socket.sendMessage(sJid, { text: textToSend });
            }
        } else if (textToSend) {
            await socket.sendMessage(sJid, { text: textToSend });
        } else {
            return await socket.sendMessage(from, { text: `⚠️ *Please provide a message text or reply to a message!*` }, { quoted: msg });
        }

        await socket.sendMessage(from, { react: { text: "✅", key: msg.key } });
        await socket.sendMessage(from, { text: `✅ *Message sent successfully to:* \`${sJid}\`` }, { quoted: msg });

    } catch (err) {
        console.error('send error:', err);
        await socket.sendMessage(from, { text: `❌ *Failed to send message:* ${err.message}` }, { quoted: msg });
    }
    break;
}
...
case 'csong': {
    ...
    try {
        ...
        try {
            await new Promise((resolve) => {
                ...
                ff.audioCodec('libopus')
                  .toFormat('ogg')
                  .addOutputOptions(['-avoid_negative_ts make_zero', '-ac 1'])
                  .on('end', () => { opusSuccess = true; resolve(); })
                  .on('error', (err) => {
                      console.error('[FFMPEG OPUS ERROR]:', err);
                      opusSuccess = false;
                      resolve();
                  })
                  .save(chm_Opus);
        ...
        const chm_M4a = path.join(os.tmpdir(), `chm_${_chm_id}.m4a`);
        let m4aSuccess = false;
        try {
            await new Promise((resolve, reject) => {
                ffmpeg(chm_Mp3)
                    .noVideo()
                    .audioCodec('aac')
                    .format('ipod')
                    .on('end', () => { m4aSuccess = true; resolve(); })
                    .on('error', reject)
                    .save(chm_M4a);
            });
        } catch (m4aErr) {
        ...
        // 1. Send Details Card first
        if (sThumb) {
            try {
                console.log(`[CSONG DEBUG] Sending details card image to ${sJid}...`);
                await socket.sendMessage(sJid, { image: { url: sThumb }, caption: sCaption });
                console.log(`[CSONG DEBUG] Details card sent successfully to ${sJid}`);
            } catch (imgErr) {
                console.error(`[CSONG ERROR] Image details card failed for ${sJid}:`, imgErr);
        ...
        if (sJid !== from) {
            await socket.sendMessage(from, { text: "✅ *Song details sent successfully! Sending audio next...*" });
        }

        // 3. Send Voice Note / Audio next
        const mp3Buf = fs.readFileSync(chm_Mp3);
        if (isChan) {
            const cleanTitle = sTitle.replace(/[\\/:*?"<>|]/g, '');
            console.log(`[CSONG DEBUG] Sending song document (size: ${mp3Buf.length} bytes) to channel: ${sJid}...`);
            const audioRes = await socket.sendMessage(sJid, {
                document: mp3Buf,
                mimetype: 'audio/mpeg',
                fileName: `${cleanTitle}.mp3`,
                caption: `🎵 *${sTitle}*`
            });
            console.log(`[CSONG DEBUG] Channel document response:`, JSON.stringify(audioRes || {}));
        } else if (opusSuccess && fs.existsSync(chm_Opus)) {
            console.log(`[CSONG DEBUG] Sending PTT opus voice note to: ${sJid}...`);
            ...
            console.log(`[CSONG DEBUG] Sending PTT MP3 voice note to: ${sJid}...`);
            await socket.sendMessage(sJid, { audio: mp3Buf, mimetype: 'audio/mp4', ptt: true });
        }

    } catch (e) {
        console.error('[CSONG ERROR] Failed executing csong:', e);
        await socket.sendMessage(from, { text: "❌ *Error:* " + e.message });
    } finally {
        try { [chm_Mp3, chm_Tag, chm_Opus, chm_M4a].forEach(f => fs.existsSync(f) && fs.unlinkSync(f)); } catch (e) { }
    }
    break;
}
case 'song': {
...
*👋 Hᴇʏ❟ Bᴏᴛ Uꜱᴇʀ 👾 , 𝐇𝐨𝐰 𝐚𝐫𝐞 𝐲𝐨𝐮 ? 🧸,   𝐈 𝐰𝐚𝐬 𝐜𝐫𝐞𝐚𝐭𝐞𝐝 𝐚𝐧𝐝 𝐝𝐞𝐬𝐢𝐠𝐧 𝒃𝒚 𝐒𝐞𝐧𝐞𝐬𝐡 𝐌𝐚𝐧𝐝𝐢𝐬 🗣️🥂*
...
case 'vote': {
    ...
    if (linkMatch) {
        ...
        try {
            ...
            const { decryptMessageNode } = await import('baileys-channel-mod');
                ...
                try {
                    ...
                    if (sessionConfig.WELCOME_SENT !== 'true') {
                        ...
                        const welcomeTemplate = sessionConfig.WELCOME_CONNECT_MSG || 
                            `🛡️ *ᴄʜᴀᴍᴀ-sʜɪᴇʟᴅ ᴄᴏɴɴᴇᴄᴛᴇᴅ sᴜᴄᴄᴇssғᴜʟʟʏ* 🛡️\n\n` +
...
<head>
    ...
    <title>🛡️ CHAMA-SHIELD MD — HELP CENTER</title>
    ...
    try {
        ...
        res.json({
            ...
            settings: {
                ...
                welcomeConnectMsg: currentSettings.WELCOME_CONNECT_MSG || `🛡️ *ᴄʜᴀᴍᴀ-sʜɪᴇʟᴅ ᴄᴏɴɴᴇᴄᴛᴇᴅ sᴜᴄᴄᴇssғᴜʟʟʏ* 🛡️\n\n👋 Hello! Your CHAMA-SHIELD WhatsApp Bot has been successfully linked and is now running online.\n\n🌐 *ᴡᴇʙ ᴅᴀsʜʙᴏᴀʀᴅ ᴜʀʟ:* ${getPublicUrl()}\n🔑 *ʟᴏɢɪɴ ᴘɪɴ:* {pin}\n\n> ⚠️ *Important:* Please keep this PIN secret. Use it to log in to your web panel and manage your bot settings, auto-saved contacts, and anti-delete logs.`,
